...
def get_sse_url():
    ...
    response_trainPosition = requests.post(
        "https://api.trafikinfo.trafikverket.se/v2/data.json", data=request_xml_trainPosition, headers=headers)
    ...
    try:
        ...
        if 'TrainPosition' in json_data['RESPONSE']['RESULT'][0]:
            ...
            for position in train_positions_data:
                ...
                logging.info(
                    f"Parsed SSE data: Train {train_number} at ({latitude}, {longitude})")
...
def stream_data():
    ...
    map = folium.Map(location=[59.3293, 18.0686], zoom_start=6)
                ...
                try:
                    ...
                    if line.startswith('data: '):
                        ...
                        for position in train_positions:
                            ...
                            folium.Marker(
                                [latitude, longitude], popup=f'Train {train_number}').add_to(map)
